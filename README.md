...
5. 將此 ioc.c 和 ioc.h 移動並覆蓋到你使用的 OSCAR/kernel 資料夾中的同名檔案，即可使用。

補充說明：
* OsIocReceivingOsApplicationRef 代表能呼叫生成的這一支 IocReceive 的 app，OsIocSendingOsApplicationRef 代表能呼叫生成的這一支 IocSend 的 app，依照需求調整 (app_lion / app2 / app3 / app4)，放一個就好。
* 如果希望所有 app 都能呼叫某一支 IOC API，就到 Ioc.c 中修改配置如下(每一行代表一個 OsIocCommunication 設定的 application 呼叫權限)。
    ![alt text](image.png)
